...
class IndexController extends BaseController {
    ...
    protected function getLoginForm(User $user) {
        ...
        $form->setAttribute('action', '/zblog.local/public/auth-doctrine/index/login/');
    ...
    protected function getRegForm(User $user) {
        $form = $this->getUserForm($user);
        $form->setAttribute('action', '/zblog.local/public/auth-doctrine/index/register/');
        $form->get('submit')->setAttribute('value', 'Register now');
        ...
        if($request->isPost()){
            ...
            if($form->isValid()){
                if($apiService->exists($user->getUsrName(), array('usrName'))){
                    $this->flashMessenger()->addErrorMessage('A user with this name exists - ' . $user->getUsrName());
    ...
    protected function sendConfirmationEmail($user) {
        ...
        $message->addTo($user->getUsrEmail())
                ->addFrom('alexander.kaykatsishvili@gmail.com')
                ->setSubject('Registration')
                ->setBody('You have successfully registered on our site!');
